...
```
start: ci permette di runnare il server nella modalità normale
...
### 7. src & index.js
Creiamo la cartella `src` che server a contenere tutti i file relativi a routes e creo al suo interno il file `index.js` nel quale scrivo di default:
```
...
### 8. promise
In Node.js nulla restituisce una promise ciò significa che non è possibile utilizzre async e await per trasformare il funzionamento asincrono ad esempio di una chiamata esterna in un funzionamento sincrono.
